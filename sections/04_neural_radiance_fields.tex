...
\subsection{Training}
\begin{itemize}
    \item NeRF is trained via supervised inverse rendering using ground-truth images.
    \item Loss:
    \[
    \mathcal{L} = \sum_{\text{rays}} \left\| C_{\text{pred}} - C_{\text{gt}} \right\|_2^2
    \]
    \item Requires known camera poses (typically estimated with COLMAP).
\end{itemize}
...
\begin{itemize}
    ...
    \item Static scenes only.
    \item Requires accurate camera pose estimation.
\end{itemize}